$(function () {
    ...
    function setTabActive(i) {
        $('.shopjieshao li button').css('color', '#666666');
        $('.shopjieshao li').css('background', '#f7f7f7');
        $('.shopjieshao li').eq(i).css('background', '#e4393c').find('button').css('color', 'white');

        $('.shopjieshaos li button').css('color', '#666666');
        $('.shopjieshaos li').css('background', '#f7f7f7');
        $('.shopjieshaos li').eq(i).css('background', '#e4393c').find('button').css('color', 'white');
    }

    function scrollToSection(i) {
        var section = $('.xuanxiangka > li').eq(i);
        if (!section.length) return;
        var top = section.offset().top - 90;
        $('html,body').animate({ scrollTop: top }, 220);
    }
    ...
    // Default to Specs & Gallery (li2); legacy intro tab (li1) is hidden.
    setTabActive(1);
    ...
    // 销售属性由 item-dynamic.js 接管（含 SKU 切换）
    $(".box-attr-2 dd").click(function () {
        if ($(".box-attr-3[data-dynamic-sale-attr]").length) {
            return;
        }
        $(this).addClass("redborder").siblings("dd").removeClass("redborder");
        if (typeof switchSkuId === "function") {
            switchSkuId();
        }
    })
    //加减
